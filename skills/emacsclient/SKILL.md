...
- Open a file: `emacsclient -s main --no-wait "/path/to/file"`
- Evaluate elisp: `emacsclient -s main --eval '(some-function)'`
- Open at a line: `emacsclient -s main --no-wait +42 "/path/to/file"`
- Byte compile a file:
  ```sh
  emacsclient -s main --eval '
...
- Always use `emacsclient -s main`, never `emacs` or `emacs --batch`.
